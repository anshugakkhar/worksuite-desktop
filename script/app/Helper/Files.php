...
class Files
{
    ...
    public static function validateUploadedFile($uploadedFile)
    {
        ...
        $forbiddenExtensions = [
            'php', 'php3', 'php4', 'php5', 'phtml', 'phar', 'sh', 'htaccess', 'pl', 'cgi', 'exe', 'bat', 'cmd', 'com', 'scr', 'dll', 'js', 'jsp', 'asp', 'aspx', 'cer', 'csr', 'jsp', 'jspx', 'war', 'jar', 'vb', 'vbs', 'wsf', 'ps1', 'ps2', 'xml'
        ];
        ...
        if (preg_match('/\.(php[0-9]?|phtml|phar|sh|pl|cgi|exe|bat|cmd|com|scr|dll|js|jsp|asp|aspx|cer|csr|jspx|war|jar|vb|vbs|wsf|ps1|ps2|xml)(\..+)?$/i', $originalName)) {
            throw new ApiException('You are not allowed to upload files with dangerous extensions', null, 422, 422, 2023);
        }

        if (in_array($extension, $forbiddenExtensions)) {
            throw new ApiException('You are not allowed to upload files with extension: ' . $extension, null, 422, 422, 2023);
        }

        if (in_array($mimeType, $forbiddenMimeTypes)) {
            throw new ApiException('You are not allowed to upload files with mime type: ' . $mimeType, null, 422, 422, 2023);
        }

        // Prevent uploading .htaccess or similar files by name
        if (strpos($originalName, '.htaccess') !== false) {
            throw new ApiException('You are not allowed to upload .htaccess files', null, 422, 422, 2023);
        }
        ...
        if (empty($extension)) {
            throw new ApiException('File must have a valid extension', null, 422, 422, 2023);
        }

        // Optionally, limit file name length
        if (strlen($uploadedFile->getClientOriginalName()) > 255) {
            throw new ApiException('File name is too long', null, 422, 422, 2023);
        }
    }
